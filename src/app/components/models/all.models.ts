// src/app/models/student.model.ts
export interface Student {
  id: number;
  name: string;
  surName: string;
  rollNo: number;
  maktabClass?: MaktabClass | null;
  age: number;
  gender: 'Male' | 'Female';
  guardianName?: string;
  phone?: string;
  imagePath?: string;
  fees: Fee[];
  status: string;
  admissionDate: string;
  attendance: Attendance[];
  dob: string;
  fatherName?: string;
  surname?: string;
  whatsapp?: string;
  classId?: number;
  address?: string;
  schoolName?: string;
  parentProfession?: string;
  preferredTime?: string;
}

// src/app/models/maktab-class.model.ts
export interface MaktabClass {
  studentCount: number;
  timing: string;
  division: string;
  id?: number;
  name: string;
  teacher?: Teacher[];
  subject?: Subject[];
  student?: Student[];
}

// src/app/models/teacher.model.ts
export interface Teacher {
  maktabClass: MaktabClass;
  id?: number;
  fullName: string;
  phone?: string;
}

// src/app/models/subject.model.ts
export interface Subject {
  id?: number;
  name: string;
  classes?: MaktabClass[];
  teacher?: Teacher[];
}

// src/app/models/attendance.model.ts
export interface Attendance {
  id: number;
  studentId: number;
  studentName: string;
  student: Student;
  className: string;
  attendanceDate: string; // YYYY-MM-DD
  attendanceStatus: 'PRESENT' | 'ABSENT' | 'LATE';
  remark: string;
  minutesLate: number | null;
}

// src/app/models/fee.model.ts
export interface Fee {
  id: number;
  studentId: number;
  month: string;
  year: number;
  status: string;
  paymentDate: Date;
  dueDate: Date;
  paymentMode: string;
  recordedBy: string;
  remark: string;
  paidAmount: number;
}

export interface BatchFeeRequest {
  studentIds: number[];
  status: string;
}

export interface FeeResponse {
  studentId: number;
  feeId: number;
  status: string;
}

export interface BatchFeeResponse {
  createdFees: FeeResponse[];
  message: string;
}

export interface FeeDTO {
  id: number;
  month: string;
  year: number;
  status: string;
  paymentDate: Date;
  paymentMode: string;
  recordedBy: string;
  remark: string;
  paidAmount: number;
}

export interface StudentFeeDTO {
  studentName: string;
  studentClass: string;
  phone: string;
  admissionDate: string;
  fees: FeeDTO[];
}

export interface ManualData {
  id?: number;                // optional because it's generated by backend
  studentName: string;
  fatherName: string;
  surname: string;
  dob: string;                // can be Date if you want to parse it
  age: number;
  gender: string;
  address: string;
  mobileNo: string;
  residential: string;
  schoolName: string;
  parentsProfession: string;
  preferredTiming: string;
  admissionDate: string;
  teacherName: string;
  maktabClass: string;
  feesDetail: string;
  division: string;
  dataJson?: string;
}

export interface AttendanceSummary {
  total: number;
  attended: number;
  absent: number;
  late: number;
}

export interface ApplicationResponse {
  title: string;
  appName: string;
  version: string;
  description: string;
}

// src/app/models/teacher-salary.model.ts
export interface TeacherSalary {
  id?: number;
  teacher: Teacher;
  teacherId: number;
  amount: number;
  salaryMonth: Date | string;
  paymentDate?: Date | string;
  paid: boolean;
  remarks?: string;
}

export interface AppNotification {
  message: string;
  type: 'success' | 'error' | 'info' | 'warning';
}

export interface ReportRow {
  id: number;
  studentId: number;
  phone: string;
  studentFullName: string;
  className: string;
  month: string;
  status: string;
  remark: string;
  paidAmount: number;
  dueAmount: number;
}

export interface ReportResponse {
  data: ReportRow[];
  currentPage: number;
  totalPages: number;
  pageSize: number;
  totalItems: number;
}

export interface ColumnConfig {
  key: keyof ReportRow | 'index';   // 'index' for serial no
  header: string;
  selected: boolean;
}

// src/app/models/evaluation.model.ts
// Evaluation master (template) — does not reference students or dates
export interface Evaluation {
  id?: number;
  name: string;           // name of the evaluation type e.g., "Midterm Math"
  description?: string;   // optional longer description
  /*maxScore?: number;      // maximum attainable score
  passingScore?: number;  // score required to pass*/
  active?: boolean;       // whether this evaluation template is active
  //remarks?: string;
}

export interface EvaluationDTO {
  name: string;
  description?: string;
  active?: boolean;
}

export interface StudentEvaluation {
  id: number;
  student: Student;
  evaluation: [{ id: number; name: string; grade?: string;}];
  grade?: number;
  remarks?: string;
  evaluationDate?: string; // YYYY-MM-DD
  evaluatorName?: string; // optional for display
  maktabClass: MaktabClass;
  classId: number;
}


